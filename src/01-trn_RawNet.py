...
def compose_spkFeat_dic(lines, model, f_desc_dic, base_dir):
    ...
    for line in tqdm(lines, desc="extracting spk feats"):
        ...
        f_desc_dic[f].seek(p)
        l = struct.unpack("i", f_desc_dic[f].read(4))[
            0
        ]  # number of samples of each utterance
        utt = np.asarray(
            struct.unpack("%df" % l, f_desc_dic[f].read(l * 4)), dtype=np.float32
        )  # read binary utterance
        spkFeat = model.predict(utt.reshape(1, -1, 1))[
            0
        ]  # extract speaker embedding from utt
...
def compose_batch(lines, f_desc_dic, dic_spk, nb_samp, base_dir):
    ...
    return (
        np.asarray(batch, dtype=np.float32).reshape(len(lines), -1, 1),
        np.asarray(ans),
    )
    ...
    for i in range(nb_batch):
        while True:
            if q.full():
                ...
                q.put(
                    compose_batch(
                        lines=lines[i * batch_size : (i + 1) * batch_size],
...
if __name__ == "__main__":
    ...
    model_pred = Model(
        inputs=model.get_layer("input_pretrn").input,
        outputs=model.get_layer("code_pretrn").output,
    )
    ...
    optimizer = eval(parser["optimizer"])(
        lr=parser["lr"], decay=0.0, amsgrad=bool(parser["amsgrad"])
    )
    ...
    if bool(parser["mg"]):
        ...
        model_mg.compile(
            optimizer=optimizer,
            loss={"s_bs_loss": simple_loss, "c_loss": zero_loss},
    ...
    for epoch in tqdm(range(parser["epoch"])):
        ...
        for b in pbar:
            pbar.set_description(
                "epoch: %d, loss: %.3f, loss_s+bs: %.3f, loss_c: %.3f"
            ...
            while True:
                if q.empty():
                    ...
                    if bool(parser["mg"]):
                        loss, loss1, loss2, acc1, acc2 = model_mg.train_on_batch(
                            [x, y], [dummy_y, dummy_y]
                        )
                    else:
                        loss, loss1, loss2, acc1, acc2 = model.train_on_batch(
                            [x, y], [dummy_y, dummy_y]
                        )
    ...
    model.load_weights(
        save_dir + "models_pretrn/best_model_on_validation.h5", by_name=True
    )
    ...
    optimizer = eval(parser["optimizer"])(
        lr=parser["lr"], decay=parser["opt_decay"], amsgrad=bool(parser["amsgrad"])
    )
    ...
    if bool(parser["mg"]):
        ...
        model_mg.compile(
            optimizer=optimizer,
            loss={"gru_s_bs_loss": simple_loss, "gru_c_loss": zero_loss},
    ...
    dev_dic_embeddings = compose_spkFeat_dic(
        lines=dev_lines, model=model_pred, f_desc_dic={}, base_dir=parser["base_dir"]
    )

    print("Extracting Embeddings from GRU model: eval set")
    eval_dic_embeddings = compose_spkFeat_dic(
        lines=eval_lines, model=model_pred, f_desc_dic={}, base_dir=parser["base_dir"]
    )

    f_embeddings = open(parser["gru_embeddings"] + "speaker_embeddings_RawNet", "wb")
    pk.dump(
        {
            "dev_dic_embeddings": dev_dic_embeddings,
            "eval_dic_embeddings": eval_dic_embeddings,
        },
        f_embeddings,
