...
def compose_batch_e2e(lines_pairs, dic_embeddings):
    ...
    return (
        np.asarray(dat_enrol, dtype=np.float32),
        np.asarray(dat_test, dtype=np.float32),
        np.asarray(ans, dtype=np.int32),
    )


def process_epoch_e2e(
    lines, q, batch_size, nb_batch_per_epoch, dic_spk2utt, dic_embeddings
):
    ...
    for i in range(nb_batch_per_epoch):
        ...
        while True:
            if q.full():
                ...
                q.put(
                    compose_batch_e2e(
                        lines_pairs=lines_pairs, dic_embeddings=dic_embeddings
                    )
                )
                break
    return
...
if __name__ == "__main__":
    ...
    if parser["optimizer"] == "SGD":
        optimizer = SGD(lr=parser["lr"], momentum=parser["momentum"])
    elif parser["optimizer"] == "Adam":
        optimizer = Adam(
            lr=parser["lr"], decay=parser["opt_decay"], amsgrad=bool(parser["amsgrad"])
        )
    elif parser["optimizer"] == "RMSprop":
        optimizer = RMSprop(lr=parser["lr"], decay=parser["opt_decay"])
