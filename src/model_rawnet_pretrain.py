...
class spk_basis_loss(Dense):
    ...
    def call(self, inputs):
        ...
        input_length = K.sum(inputs_x**2.0, axis=1, keepdims=True) ** 0.5
        input_length /= self.s**0.5
        input_length += 0.0001

        kernel_length = K.sum(self.kernel**2.0, axis=0, keepdims=True) ** 0.5
        ...
        negative_mask = tf.fill([self.units, self.units], 1.0) - tf.eye(self.units)
        # shape = [#spk, #spk]

        loss_BS = K.mean(
            tf.matmul(
                kernel_norm, kernel_norm, adjoint_a=True  # transpose second matrix
            )
            * negative_mask
        )
...
class CenterLossLayer(Layer):
    ...
    def call(self, x, mask=None):
        ...
        delta_centers = K.dot(
            K.transpose(x[1]), (K.dot(x[1], self.centers) - x[0])
        )  # 10x2
        ...
        self.result = K.sum(
            self.result**2, axis=1, keepdims=True
        )  # / K.dot(x[1], center_counts)
...
def residual_block_conv(
    input_tensor, filters=[], initializer=None, regularizer=None, base_name=None
):
    ...
    if K.int_shape(input_tensor)[-1] != K.int_shape(x)[-1]:
        input_tensor = Conv1D(
            ...
            kernel_regularizer=regularizer,
            padding="same",
            name=base_name + "_transform",
        )(input_tensor)
        input_tensor = BatchNormalization(name=base_name + "_BN_transform")(
            input_tensor
        )
...
def get_model(argDic):
    ...
    for i in range(1, 3):
        x = residual_block_conv(
            x,
            argDic["nb_conv_filt"][0],
    ...
    c_out = CenterLossLayer(
        alpha=argDic["c_alpha"],
